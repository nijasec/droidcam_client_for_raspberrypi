...
void * jfif_buffer(unsigned char *buffer,long length)
{
	JFIF *jfif   = NULL;
	int   header = 0;
    int   type   = 0;
	...
	type=buffer[1];
	 if ((type == 0xd8) || (type == 0xd9) || (type == 0x01) || (type >= 0xd0 && type <= 0xd7)) {
		...
		printf("size io image is :%d",size);
	...
	*/
	 ...
	 while (1) {
        do { header = *(buffer+offset); offset++;} while (offset< length && header != 0xff); // get header
        do { type   = *(buffer+offset);offset++;} while (offset < length && type   == 0xff); // get type
       /* if (header == EOF || type == EOF) {
        ...
        if ((type == 0xd8) || (type == 0xd9) || (type == 0x01) || (type >= 0xd0 && type <= 0xd7)) {
            ...
            size  = *(buffer+offset) << 8; offset++;//offset++;
            size |= *(buffer+offset)<< 0;offset++;
		//	buffer++;offset++;
            size -= 2;
        }
		///printf("\nsize of image is :%d",size);
        ...
        case 0xda: // SOS
			...
			//printf("comp_num:%d", jfif->comp_num );
            for (i=0; i<jfif->comp_num; i++) {
	...
	//printf("ofs:%d",offset);
    jfif->datalen = length - offset;
    jfif->databuf = malloc(jfif->datalen);
    if (jfif->databuf) {
       // fseek(fp, offset, SEEK_SET);
	   memcpy(jfif->databuf,buffer+offset,jfif->datalen);
